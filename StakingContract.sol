...
contract StakingSmartContract is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
     ...
     * stake only 
     */
    function approve() external nonReentrant returns (bool){
        uint256 amount = stakedToken.balanceOf(msg.sender); 
        bool gotApprove = stakedToken.approve(address(this), amount); 
        return gotApprove; 
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            stakedToken.approve(address(this), _amount); 
    ...
    function claimRewardToken(uint256 _amount) external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        uint256 pending = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);

        require(pending >= _amount, "Amount to withdraw too high");
