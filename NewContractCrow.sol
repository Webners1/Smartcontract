...
contract CROGECOIN is IBEP20, Auth {
   ...
   struct BuyFee {
    ...
    address private _marketingFeeReceiver = 0x3cCCD297e91d2811Bda51C33B1d5D096179Ab662;
 ...
 function setBuyFee(uint16 liq, uint16 market, uint16 reward, uint16 tax) external onlyOwner {
               require(liq<= 3,"cant set Liqudity for more than 3%");
        require(reward<= 1,"cant set Liqudity for more than 1%");
        require(tax<= 2,"cant set Liqudity for more than 2%");
        require(market<= 2,"cant set Liqudity for more than 2%");
        buyFee.liquidityFee = liq;
        buyFee.marketingFee = market;
        buyFee.teamFee = reward;
        buyFee.taxFee = tax;
    }
    function setSellFee(uint16 liq, uint16 market, uint16 reward, uint16 tax) external onlyOwner {
        require(liq<= 5,"cant set Liqudity for more than 5%");
        require(reward<= 2,"cant set Liqudity for more than 2%");
        require(tax<= 3,"cant set Liqudity for more than 3%");
        require(market<= 2,"cant set Liqudity for more than 2%");
        sellFee.liquidityFee = liq;
        sellFee.marketingFee = market;
        sellFee.teamFee = reward;
        sellFee.taxFee = tax;
    }
    ...
    function gettaxFee(bool selling) public view returns (uint256) {
        if(launchedAt + 1 >= block.number){ return feeDenominator.sub(1); }
        if(selling) { return taxFee.add(1); }
        return taxFee;
    }
  function removeAllFee() private {
    ...
    function setBuy() private {
        taxFee = buyFee.taxFee;
        liquidityFee = buyFee.liquidityFee;
        teamFee = buyFee.teamFee;
        marketingFee = buyFee.marketingFee;
    }

    function setSell() private {

        taxFee = sellFee.taxFee;
        liquidityFee = sellFee.liquidityFee;
        teamFee = sellFee.teamFee;
        marketingFee = sellFee.marketingFee;
    }
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(gettaxFee(receiver == pair)).div(feeDenominator);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = contractTokenBalance.mul(liquidityFee).div(taxFee).div(2);
        ...
        uint256 totalBNBFee = taxFee.sub(liquidityFee.div(2));
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function setFees(uint256 _liquidityFee, uint256 _teamFee, uint256 marketingFee, uint256 _feeDenominator) external authorized {
        liquidityFee = liquidityFee;
        teamFee = _teamFee;
        marketingFee = marketingFee;
        taxFee = liquidityFee.add(_teamFee).add(marketingFee);
        feeDenominator = _feeDenominator;
    }

    function setFeeReceiver(address __marketingFeeReceiver, address _teamFeeReceiver) external authorized {
